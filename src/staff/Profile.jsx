  ...
  //  append image
   if (profile.image && typeof profile.image === 'string') {
    try {
      ...
      const imageResponse = await axios.get(profile.image, { responseType: 'blob' });
      ...
      try {
        ...
        usernameData.append('new_username', myUser.username);
        const usernameUpdateResponse = await axios.put(updateUsername, usernameData,{
          headers:{"Authorization": `Bearer ${localStorage.getItem('authTokens')}`},
    });
        console.log(usernameUpdateResponse);
      } catch (error) {
